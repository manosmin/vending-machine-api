...
export const deposit = async (req, res) => {
  ...
  if (!validCoins.includes(amount)) {
    return res.status(400).json({ message: 'Invalid coin value. Accepted coins: 5, 10, 20, 50, 100.' });
  }
...
export const buy = async (req, res) => {
  ...
  try {
    ...
    if (!product || product.amountAvailable < amount) {
      return res.status(400).json({ message: 'Insufficient product availability' });
    }
...
export const reset = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error resetting deposit', error });
  }
};
